...
| 2.2.4 | Build `<CreateQuizModal>` — shadcn Dialog, multi-step: (1) select materials/folder, (2) question count + time limit via React Hook Form + Zod, (3) mode (solo/competitive); calls Edge Function via `supabase.functions.invoke('generate-quiz', ...)` | `[x]`  | 1.1.6, 1.1.8, 2.2.1, 2.1.5  | `src/components/CreateQuizModal.tsx` — 3-step flow with loading state                       |
| 2.2.5 | Show success feedback — shadcn Toast with "Quiz ready!" and "Play Now" / "Send Challenge" CTA                                                                                                                                                         | `[x]`  | 2.2.4                       | Auto-navigates to `/quiz/:id` on success                                                    |
